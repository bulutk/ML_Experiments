...
public sealed class ObjectPool : MonoBehaviour
{
	...
	Dictionary<Component, Stack<Component>> objectLookup = new Dictionary<Component, Stack<Component>>();
	...
	public static void CreatePool<T>(T prefab) where T : Component
	{
		if (!instance.objectLookup.ContainsKey(prefab))
			instance.objectLookup.Add(prefab, new Stack<Component>());
	}
	...
	{
		if(obj == null || obj.gameObject == null) // if someone else killed them first.
			return;

		if (instance.prefabLookup.ContainsKey(obj))
		{
			instance.objectLookup[instance.prefabLookup[obj]].Push(obj);
